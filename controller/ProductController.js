...
const uploadCatalogue = async (req, res) => {
      ...
      let prod_details = await models.ProductDetails.create({ desc,unit_price,qty,weight,currency,
        qty_measure,unit}, { fields: ['desc','unit_price','qty','weight','currency','qty_measure','unit'] })
      console.log("prod_details===",prod_details);
      //let prod = await models.Product.create({ title,desc,cat_id,type_id,unit_price,qty,weight,img_url,shape_id,original_flavour_id }, { fields: ['title','desc','cat_id','type_id','unit_price','qty','weight','img_url','shape_id','original_flavour_id'] })
      /*.then(prod => {
